...
const areModifierSelectionsEqual = (
  ...
  return keysA.every((key) => {
    ...
    return (
      valA.every((id) => valB.includes(id)) &&
      valB.every((id) => valA.includes(id))
    );
  });
};
  ...
  const toggleOption = (optionId: string, group: ModifierGroup) => {
    setSelectedOptionIds((prev) => {
      ...
      if (isSelected) {
        ...
        const optionsInGroupCount = prev.filter((id) =>
          group.options?.some((o) => o.id === id),
        ).length;
              ...
              <View className="flex-row justify-between items-baseline mb-6">
                ...
                <View className="bg-surface-container px-3 py-1 rounded-full">
                  <Text className="text-[11px] font-medium text-on-surface-variant">
                    {group.minSelections > 0 ? 'Required' : 'Optional'}, max{' '}
                    {group.maxSelections}
                  </Text>
                </View>
              </View>
                  ...
                  return (
                    ...
                    >
                      <View className="flex-row items-center space-x-4 flex-1">
                        <View
                          className={`relative flex items-center justify-center w-6 h-6 border border-outline-variant bg-surface ${
