...
function PromotionFormContent({
  ...
  // scope is always 'restaurant' and trigger is always 'auto_apply' for the
  // restaurant portal. Coupon-code promotions are admin-only: they require
  // coupon-code generation/management, which the restaurant portal does not
  // expose. (Legacy coupon promotions still display correctly when editing —
  // trigger is immutable on update.)
  const trigger: PromotionTrigger = existing?.trigger ?? 'auto_apply';
          ...
          <Field label="Scope" required>
            <div className="grid grid-cols-1 gap-2">
              <ChoiceCard
                ...
                description="Applies only to this restaurant."
                onClick={() => {}}
              />
            </div>
          </Field>
          ...
          <Field label="Trigger" required>
            <div className="grid grid-cols-1 gap-2">
              <ChoiceCard
                ...
                title={trigger === 'coupon_code' ? 'Coupon code' : 'Auto-apply'}
                description={
                  trigger === 'coupon_code'
