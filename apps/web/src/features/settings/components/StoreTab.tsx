...
function LocationMarker() {
  ...
  const lat = useWatch<UpdateRestaurantFormValues, 'latitude'>({
    name: 'latitude',
  });
  const lng = useWatch<UpdateRestaurantFormValues, 'longitude'>({
    name: 'longitude',
  });
...
export function StoreTab() {
  ...
  const handleGeocodeAddress = async () => {
    ...
    try {
      ...
      if (data.features && data.features.length > 0) {
        ...
        alert(
          'Could not locate this address. Please refine your search and try again.',
        );
      }
    } catch (error) {
  ...
  const clearImage = (event: MouseEvent<HTMLButtonElement>) => {
    ...
    methods.setValue('coverImageUrl', '', {
      shouldDirty: true,
      shouldValidate: true,
    });
  };
  ...
  if (isLoading) {
    return (
      <div className="text-on-surface-variant animate-pulse p-6">
        Loading store data...
      </div>
    );
  }
    ...
    return (
      <div className="bg-surface-container-low rounded-3xl p-8 text-center border border-dashed border-outline-variant">
        ...
        <p className="text-on-surface-variant max-w-md mx-auto">
          You don't have a registered restaurant yet. Please register your
          business first.
        </p>
      </div>
    );
      ...
      <div className="space-y-6">
        ...
        <section className="bg-surface-container-lowest rounded-3xl p-6 md:p-8">
          ...
          <p className="text-sm text-on-surface-variant mb-6">
            Type your address above and click{' '}
            <span className="font-medium">Locate on Map</span> to pin your exact
            restaurant location. This is used for delivery estimates and
            coverage zones.
          </p>
          ...
          <div className="relative aspect-video lg:aspect-[21/9] bg-surface-container rounded-2xl overflow-hidden shadow-sm border border-outline-variant/10 z-0">
            <MapContainer
              center={[
                restaurant.latitude || DEFAULT_CENTER.lat,
                restaurant.longitude || DEFAULT_CENTER.lng,
              ]}
              zoom={13}
        ...
        <section className="bg-surface-container-lowest rounded-3xl p-6 md:p-8">
          ...
          <p className="text-sm text-on-surface-variant mb-6">
            Upload a high-quality image to represent your restaurant to
            customers.
          </p>
            ...
            {currentCoverUrl ? (
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-6">
                ...
                <p className="text-sm text-on-surface-variant mt-1">
                  {isUploading
                    ? 'Please wait'
                    : 'Click or drag a JPG/PNG. Max 5 MB'}
                </p>
              </div>
            )}
          </div>
